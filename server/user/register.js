...
router.post("/", (req, res) => {
  ...
  if ((firstname, lastname, username, email, password, isName(firstname), isName(lastname), isUsername(username), isEmail(email), isPassword(password))) {
      ...
      bcrypt.hash(password, 10, (err, hash) => {
        ...
        db.query("SELECT COUNT(*) AS count FROM `users` WHERE `username` = ? OR `email` = ? LIMIT 1;", [username, email], (error, rslt) => {
          ...
          if (err) {
            ...
            db.query(
              "INSERT INTO users(firstname,lastname,username,email,password,token,last_connection) VALUES (?,?,?,?,?,?,0);",
              [firstname, lastname, username, email, hash, token],
              (err, result) => {
                if (err) {
                  console.log(err);
                } else {
                  if (send_Email(email, "Confirm account", `<p>To activate your account please click <a href="http://localhost:3000/confirm/${token}">Here</a></p>`)) {
