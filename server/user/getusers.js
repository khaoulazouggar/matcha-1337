...
router.get("/", isUserAuth, (req, res) => {
    ...
    db.query(sqlgender, id, (err, rows) => {
      ...
      {
        const sqlSelect = "SELECT * FROM `users` WHERE users.id  NOT IN (SELECT users.id from users left join block on (users.id = block.blocked OR users.id = block.blocker) WHERE (block.blocker = ? or block.blocked = ?)) AND gender = ? AND users.id <> ?";
